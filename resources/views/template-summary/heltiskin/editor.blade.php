...
@section('content')
    <form action="{{ route('template-summary.heltiskin.export') }}" method="POST" id="heltiskinTemplateForm"
    ...
    <script>
        ...
        function collectFormValues() {
            ...
            document.querySelectorAll(
                    '#heltiskinTemplateForm input[name]:not([type="file"]), #heltiskinTemplateForm textarea[name], #heltiskinTemplateForm select[name]'
                )
                .forEach(input => {
        ...
        function collectDraftState() {
            ...
            document.querySelectorAll(
                    '#heltiskinTemplateForm input[name], #heltiskinTemplateForm textarea[name], #heltiskinTemplateForm select[name]')
                .forEach(field => {
                    if (field.disabled) {
        ...
        async function saveDraft() {
            ...
            try {
                const form = document.getElementById('heltiskinTemplateForm');
                const formData = new FormData(form);
                formData.delete('_token');
                formData.append('_token', CSRF_TOKEN);
                const state = collectDraftState();

                const product = (state.form_values.judul_nama_produk || '').trim() || 'Heltiskin Cream';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // ===========================================
            const heltiskinForm = document.getElementById('heltiskinTemplateForm');
            if (heltiskinForm) {
                heltiskinForm.addEventListener('submit', function() {
                    showExportModal();
                    document.querySelectorAll('.mixing-table-item').forEach(tableItem => {
                        const tableUid = getTableUidFromItem(tableItem);
                        if (!tableUid) return;
        ...
        function showExportModal() {
            ...
            if (!tokenInput) {
                ...
                document.getElementById('heltiskinTemplateForm').appendChild(tokenInput);
            }
