    ...
    <script>
        ...
        function collectFormValues() {
            ...
            document.querySelectorAll(
                    '#nutracareTemplateForm input[name]:not([type="file"]), #nutracareTemplateForm textarea[name], #nutracareTemplateForm select[name]'
                )
                .forEach(input => {
        ...
        function collectDraftState() {
            ...
            document.querySelectorAll(
                    '#nutracareTemplateForm input[name], #nutracareTemplateForm textarea[name], #nutracareTemplateForm select[name]')
                .forEach(field => {
                    if (field.disabled) {
        ...
        async function saveDraft() {
            ...
            try {
                const form = document.getElementById('nutracareTemplateForm');
                const formData = new FormData(form);
                formData.delete('_token');
                formData.append('_token', CSRF_TOKEN);
                const state = collectDraftState();

                const product = (state.form_values.judul_nama_produk || '').trim() || 'Nutracare EPO 500 Soft Capsule';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (nutracareForm) {
                nutracareForm.addEventListener('submit', function() {
                    showExportModal();
                    document.querySelectorAll('.mixing-table-item').forEach(tableItem => {
                        const tableUid = getTableUidFromItem(tableItem);
                        if (!tableUid) return;
        ...
        function showExportModal() {
            ...
            if (!tokenInput) {
                ...
                document.getElementById('nutracareTemplateForm').appendChild(tokenInput);
            }
