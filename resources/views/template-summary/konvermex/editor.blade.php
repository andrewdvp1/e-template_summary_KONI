...
@section('content')
    <form action="{{ route('template-summary.konvermex.export') }}" method="POST" id="konvermexTemplateForm"
    ...
    <script>
        ...
        function collectFormValues() {
            ...
            document.querySelectorAll(
                    '#konvermexTemplateForm input[name]:not([type="file"]), #konvermexTemplateForm textarea[name], #konvermexTemplateForm select[name]'
                )
                .forEach(input => {
        ...
        function collectDraftState() {
            ...
            document.querySelectorAll(
                    '#konvermexTemplateForm input[name], #konvermexTemplateForm textarea[name], #konvermexTemplateForm select[name]')
                .forEach(field => {
                    if (field.disabled) {
        ...
        async function saveDraft() {
            ...
            try {
                const form = document.getElementById('konvermexTemplateForm');
                const formData = new FormData(form);
                formData.delete('_token');
                formData.append('_token', CSRF_TOKEN);
                const state = collectDraftState();

                const product = (state.form_values.judul_nama_produk || '').trim() || 'Konvermex 125 Suspensi';
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // ===========================================
            const konvermexForm = document.getElementById('konvermexTemplateForm');
            if (konvermexForm) {
                konvermexForm.addEventListener('submit', function() {
                    showExportModal();
                    document.querySelectorAll('.mixing-table-item').forEach(tableItem => {
                        const tableUid = getTableUidFromItem(tableItem);
                        if (!tableUid) return;
        ...
        function showExportModal() {
            ...
            if (!tokenInput) {
                ...
                document.getElementById('konvermexTemplateForm').appendChild(tokenInput);
            }
